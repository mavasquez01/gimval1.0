...
class Alumna_model extends CI_Model
{
    ...
    public function AL_07($rut_alumna)
    {
        ...
        if (empty($rut_alumna)) {
            return json_encode([
                'success' => false,
                'error' => 'No se proporcionó el RUT de la alumna.'
            ]);
        }
        ...
        SELECT
            ...
            (
                SELECT COUNT(*)
                FROM reserva r2
                WHERE r2.id_bloque = b.id_bloque
                  AND r2.vigente = 1
            ) AS cupos_ocupados,
            ...
            (
                SELECT rt.id_rutina
                FROM rutina rt
                WHERE rt.id_bloque = b.id_bloque
                  AND rt.vigente = 1
                ORDER BY rt.id_rutina DESC
                LIMIT 1
            ) AS id_rutina

        FROM reserva r

        INNER JOIN bloque_horario b
            ON r.id_bloque = b.id_bloque

        INNER JOIN profesor p
            ON b.rut_profesor = p.rut
    ...
    ";
        ...
        $query = $this->db->query($sql, [$rut_alumna]);
